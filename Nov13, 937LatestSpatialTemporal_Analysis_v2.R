...
# Outlier analysis
# This is to remove any responses that equal to or greater than 10 seconds.
dat = dat %>% 
  filter(duration_response >= 10)
...
# estimates in both control and experimental conditions
    #Since the interaction analyses for H3 will produce the single effect of H1,
    #We will use the same model for both



Model1 = lmer(duration_response ~ total_length * Condition  + (1 | Participant_ID),
              data = dat_timing)
summary(Model1)
...
# Note: here we are using the same model as H1 - and calling for the same thing once again.
# Although this step is not necessary, we have called this summary for a second time so that our 
# output retains a sequential flow. 
